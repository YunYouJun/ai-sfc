...
describe('generateCoupletsDirect', () => {
  it('成功时返回 ok+couplets，并直连用户填写的 baseURL（带 Bearer token）', async () => {
    const fetchMock = mockFetch({
      body: { choices: [{ message: { content: '{"上联":"a","下联":"b","横批":"c","总结":"福"}' } }] },
    })

    const result = await generateCoupletsDirect('家人平安', provider)
    expect(result).toEqual({ ok: true, couplets: { 上联: 'a', 下联: 'b', 横批: 'c', 总结: '福' } })
  ...
  it('非 2xx 状态返回 ok:false + statusCode', async () => {
  ...
  it('返回内容无法解析为春联时返回 ok:false', async () => {
    vi.spyOn(console, 'error').mockImplementation(() => {})
